...
import {YamahaReceiver} from "./index";
...
export class YamahaReceiverClientImpl implements YamahaReceiverClient {
  ...
  private log: Logger;
  private inputs: {[deviceInputName: string]: string};

  constructor(address: string, inputs: {[deviceInputName: string]: string;}) {
    this.log = createLogger('YamahaReceiverClientImpl');
    this.address = address;
    this.inputs = inputs;
  }

  changeInput(input: string): Promise<void> {
    const newInput = this.inputs[input];
    if (newInput) {
      input = newInput;
    }
  ...
  buttonPress(buttonName: string): Promise<void> {
    switch (buttonName.toUpperCase()) {
      ...
      case 'VOLUMEDOWN':
        return this.changeVolume(YamahaReceiver.Zone.MAIN, -0.5);
      default:
        return Promise.reject(new Error(`Invalid button name: ${buttonName}`));
    }
  }
